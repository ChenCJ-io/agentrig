...
> RFC ID：AR-RFC-0003
>
> 状态：Accepted / Partially implemented（Phase 0 与 Phase 3 基本落地；Phase 1 顶栏选择器与
> Phase 2 会话过滤未落地，逐条状态见 §12）
>
> 版本：0.2
>
> 日期：2026-08-11（提出）／2026-09-09（状态更新）
>
...
顶栏目前仍是**只读的当前被测 Agent 上下文展示**，切换 Target 经 `/targets` 列表导航完成；
在 §12 中该项标记为未落地前，不要把顶栏解释为已具备内联切换能力。
...
- `web/app/pages/v2/assistant-page.tsx`：新会话把路由 Target ID 写入
  ...
  跨 Target 的最近会话；
- 评测助手规划时优先把 `AssistantSession.workspace_id` 匹配为 Target ID，Plan 最终又在
...
当前 `workspace_id` 已被评测助手当作 Target ID 使用。P0 保留该存储与线上契约以避免
...
| Curator/Judge 内置实现定义 | 否 | 评判与模拟是平台级执行能力 |
| Curator/Judge 实际调用证据 | 是 | 属于当前 Session/Run 的调用必须联动 |
...
- [x] 接受 URL `:targetId` 作为工作区 Target 权威来源；
- [x] 接受 P0 继续使用 `AssistantSession.workspace_id` 表达会话 Target 绑定；
- [x] 接受切换 Target 时不保留跨 Target 的选中会话与未保存表单；
- [x] 接受多版本未显式选择时显示“多版本/未指定”，不默认第一项；
- [x] 接受环境元数据缺失时显示未知或省略，不猜测。
...
| 决策项 | 状态 | 落点 |
|---|---|---|
| URL `:targetId` 为工作区 Target 权威来源 | Implemented | `web/app/pages/workspace-router.tsx` 解码 `targetId` 并下传给各工作区页面 |
| 顶栏展示真实 Target 身份与运行时信息，不再硬编码“本机”、不默认 `versions[0]` | Implemented | `web/app/components/shell/app-shell.tsx`（`target?.name` / `endpoint` / `driver_type` / `targetRuntimeVersion()`） |
| Target 范围查询的 query key 绑定 `targetId` | Implemented（评测域） | `evaluation-page.tsx` 的 `runs` / `run` / `run-summary` / `run-cells` / `cell` / `run-preview` 均含 `targetId` |
| AssistantSession 绑定 Target 工作区 | Implemented | `assistant/schemas.py` 的 `workspace_id` |
| 服务端守住 Plan 与工作区 Target 一致性 | Implemented | `assistant/basic_runtime.py` 的 `_require_workspace_target()`：计划选择的 Target 与 `workspace_id` 不一致即拒绝 |
| 工作区一致性回归测试 | Implemented | `web/e2e/assistant-workspace.spec.ts`、`web/e2e/evaluation-workspace.spec.ts` |
| **顶栏内联 Target picker（Phase 1）** | **Pending** | 顶栏为只读展示，切换仍经 `/targets` 列表导航 |
| **`GET /api/v2/assistant/sessions?workspace_id=` 过滤（§6.1 P0 API）** | **Pending** | `v2_api.py` 的 `list_sessions` 只有 `limit` / `offset`；`AssistantService.list_sessions` 同样未接受该过滤 |
| **`POST /assistant/sessions` 校验 `workspace_id` 对应项目内真实 Target** | **Pending** | 当前只在计划选择阶段比对，创建会话时不校验 |
| 助手会话 query key 从 `['v2','sessions']` 调整为含 `targetId`（§6.2） | Pending | `assistant-page.tsx` 仍为 `["v2", "sessions"]` |

继续实施时按 §9 的 Phase 1 与 Phase 2 推进；完成后把本文状态改为 Implemented，并在
[智能评测助手架构](./03-智能评测助手架构.md) 同步会话过滤契约。
