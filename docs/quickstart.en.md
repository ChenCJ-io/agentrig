...
To stream live traffic in for production regression, see the Chinese
[production trace onboarding guide](./04-Trace接入指南.md); the enable switch and endpoints are
summarised under "Ingest production traces" below.
...
| Surface | Default URL | Authentication |
|---|---|---|
| Web | `http://127.0.0.1:8000/` | public static assets |
| HTTP API | `http://127.0.0.1:8000/api/` | deployment token or project `agrp_` key |
| Assistant API | `http://127.0.0.1:8000/api/v2/` | same |
| Controller MCP | `http://127.0.0.1:8000/mcp/` | same |
| Tested-agent tool proxy | `http://127.0.0.1:8000/proxy` | scope token |
| OTLP trace receiver | `http://127.0.0.1:8000/v1/traces` | ingest-source token (**never** the deployment token) |
| OpenAI-compatible gateway | `http://127.0.0.1:8000/gateway/{source_id}/v1/…` | ingest-source token |

`0.3.0a0` removed the external multi-agent collaboration integration, so the `/mcp/manager/`,
`/mcp/curator/`, and `/mcp/judge/` role endpoints no longer exist. External controllers use
`/mcp/`.
...
Production evidence ingest is off by default:
...
Once enabled, create an ingest source from the Web "onboarding" page and copy the generated
configuration. Both lanes land in the same trace storage and policy:

| Lane | Endpoint | Best for |
|---|---|---|
| Out-of-band OTel export | `POST /v1/traces` (protobuf and JSON, gzip supported) | production default; never blocks the request path |
| OpenAI-compatible gateway | `POST /gateway/{source_id}/v1/{suffix}` | zero-code onboarding; development and staging |

Each source carries its own token, rate limit, `retention_days`, and redaction policy. No message
bodies are stored unless you explicitly opt in; `save_full_content` stores redacted full
conversations and tool arguments so a trace can be turned into a replayable case with Sample
drafts.
...
The version-pinned AgentScope 2.0.6 and PostgreSQL checks require controlled
