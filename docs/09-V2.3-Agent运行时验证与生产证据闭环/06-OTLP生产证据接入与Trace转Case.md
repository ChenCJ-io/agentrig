...
>
> 后继：本文的接收侧契约仍然有效，但接入车道与正文策略已被
> [AR-RFC-0005](../11-一站式Trace接入与生产回归闭环-RFC.md) 扩展——新增 OTLP/JSON 编码、
> OpenAI 兼容网关车道、`save_full_content` 第三档策略与 Sample 草稿自动化。
> 用户操作入口统一见[生产 Trace 接入指南](../04-Trace接入指南.md)。
