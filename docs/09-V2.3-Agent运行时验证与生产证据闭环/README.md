...
> 当前实现权威边界：`../00-总体架构.md`
>
> 前置方案：AR-RFC-0003（可复现交付与质量门禁，已实施）

本目录把 AgentRig 该阶段工作分成一份总方案和多份专题设计，并提供统一实施、验收矩阵与运行
手册。相应代码、迁移、API/CLI、Web 入口和确定性测试均已实现；本地验收、真实 Reference
Backend 浏览器链路和 PostgreSQL 17.7 双 Worker/retention 集成已完成。AgentScope v2.0.6 Live
acceptance 已提供可执行入口，但在对应环境实际执行前不标记为 `Verified`。

> 注：原 02 号专题（外部协作运行时双基线兼容）与相关角色协作能力已按 AR-RFC-0004 从代码库
> 整体移除；本包其余文档仍是当前实现的权威记录。
...
- 首次无 live 变量的全量套件产生的 skip 需要外部环境，不计为通过；
...
- 外部待执行：AgentScope v2.0.6 与目标部署容量/SLO benchmark；
