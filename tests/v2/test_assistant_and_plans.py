...
async def test_confirmed_plan_detects_asset_drift(services: ServiceContainer) -> None:
    plan_id, confirmation_event_id, _session_id = await _plan(services)
    await services.evaluation_plans.confirm(
        plan_id,
        EvaluationPlanConfirm(
            confirmation_event_id=confirmation_event_id,
    ...
    stale = await services.evaluation_plans.get(plan_id)
    assert stale.last_error is not None
    assert stale.last_error["code"] == ErrorCode.PLAN_STALE.value
