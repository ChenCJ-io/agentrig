...
from ..infrastructure.database.repositories.samples import SqlSampleRepository
from ..infrastructure.database.session import Database
from ..infrastructure.http_policy import TargetHttpPolicy
from ..runs.redactor import Redactor
from ..tool_results import SampleCreate, SampleService
from .otlp import (
    count_spans,
    normalized_spans,
...
class ProductionEvidenceService:
    ...
    async def create_source(
        ...
        if value.gateway is not None:
            self._http_policy.validate_url(value.gateway.upstream_base_url)
        ...
        row = IngestSourceORM(
            ...
            gateway=(
                value.gateway.model_dump(mode="json")
                if value.gateway is not None
                else None
            ),
    ...
    async def authenticate_gateway_source(
        self,
        source_id: str,
        token: str,
    ) -> IngestSourceView:
        """Resolve and verify an openai_gateway source by id across projects."""

        if not self._config.enabled:
            raise AgentRigError(ErrorCode.PERMISSION_DENIED, "production ingest is disabled")
        async with self._database.session() as session:
            row = await session.get(IngestSourceORM, source_id)
        if (
            row is None
            or not row.enabled
            or row.source_type != "openai_gateway"
            or row.gateway is None
            or row.api_key_hash != self._token_hash(token)
        ):
            raise AgentRigError(
                ErrorCode.PERMISSION_DENIED,
                "unknown, disabled, or non-gateway ingest source",
            )
        self._check_rate_limit(self._source_view(row))
        return self._source_view(row)
        ...
        *,
        ...
        try:
            if payload_format == "json":
                request = parse_export_request_json(body)
            else:
                request = parse_export_request(body)
        except (DecodeError, ValueError, json_format.ParseError) as exc:
            raise AgentRigError(
                ErrorCode.VALIDATION_ERROR,
                f"malformed OTLP {payload_format} payload",
            ) from exc
    ...
    async def preview_case_draft(
        ...
        tool_sample_plan: list[ToolSamplePlanItem] = []
        if value.create_tool_samples:
            for span_id in selected_ids:
                span = available[span_id]
                if (
                    span.tool_call
                    and span.tool_call.get("tool_name")
                    and span.tool_result
                    and span.tool_result.get("result_redacted")
                ):
                    tool_sample_plan.append(
                        ToolSamplePlanItem(
                            span_id=span_id,
                            tool_name=str(span.tool_call["tool_name"]),
                        )
                    )
        mapping = {
            "trace_id": trace_id,
            ...
            "tool_sample_plan": [item.model_dump(mode="json") for item in tool_sample_plan],
        }
        return TraceCaseDraftPreview(
            source_trace_id=trace_id,
            selected_span_ids=selected_ids,
            tool_sample_plan=tool_sample_plan,
    ...
    async def create_case_draft(
                    ...
                    TestTurn.model_validate(
                        {
                            ...
                            "assertions": [
                                ...
                                for tool_name in sorted(
                                    {
                                        item.tool_name
                                        for item in preview.tool_sample_plan
                                    }
                                )
                            ],
                        }
        ...
        detail = await self.get_trace(project_id, trace_id)
        spans_by_id = {span.id: span for span in detail.spans}
        sample_ids: list[str] = []
        if preview.tool_sample_plan:
            sample_service = SampleService(
                SqlSampleRepository(self._database, project_id=project_id)
            )
            for item in preview.tool_sample_plan:
                span = spans_by_id[item.span_id]
                assert span.tool_call is not None and span.tool_result is not None
                sample = await sample_service.create(
                    SampleCreate(
                        name=f"Trace {trace_id[:16]} {item.tool_name}",
                        tool_name=item.tool_name,
                        content=_parse_captured_json(
                            span.tool_result["result_redacted"]
                        ),
                        match_arguments=_captured_arguments(span.tool_call),
                        supported_versions=value.target_versions,
                    )
                )
    ...
    async def review_case_lineage(
        ...
        async with self._database.session() as session:
            ...
            if value.status == "approved":
                ...
                for sample_id in row.draft_sample_ids or []:
                    sample = await session.scalar(
                        select(SampleORM).where(
                            SampleORM.id == sample_id,
                            SampleORM.project_id == project_id,
                        )
                    )
                    if sample is None or sample.status != "approved":
                        raise AgentRigError(
                            ErrorCode.CONFLICT,
                            "lineage approval requires every draft Sample to be approved",
                        )
        ...
        async with self._database.session() as session:
            ...
            if trace is None:
                ...
                trace = ProductionTraceORM(
                    ...
                    input_full_redacted=value.get("input_full_redacted"),
                    output_full_redacted=value.get("output_full_redacted"),
            ...
            known_end = trace.ended_at or trace.started_at
            if known_end.tzinfo is None:
                # SQLite hands naive datetimes back; the ingest values are UTC-aware.
                known_end = known_end.replace(tzinfo=timezone.utc)
            if value["ended_at"] > known_end:
...
def _parse_captured_json(value: str) -> Any:
    try:
...
def _captured_arguments(tool_call: dict[str, Any]) -> dict[str, Any]:
    raw = tool_call.get("arguments_redacted")
