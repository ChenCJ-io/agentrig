...
class AgentRigProxy:
    ...
    async def call_tool(
        ...
        if self.mock_policy is not None and self.mock_policy.should_mock(name, arguments):
            content = _to_content(self.mock_policy.generate(name, arguments))
            if self.trace_sink is not None:
                self.trace_sink.record(TraceEntry(name, arguments, source="mock"))
            return content
        ...
        try:
            ...
            if self.trace_sink is not None:
                self.trace_sink.record(
                    TraceEntry(name, arguments, source="real", is_error=True)
                )
            return _error_content(f"backend error: {e!r}")
        if self.trace_sink is not None:
            self.trace_sink.record(
                TraceEntry(name, arguments, source="real", is_error=result.isError)
            )
...
def _to_content(value: Any) -> list[types.ContentBlock]:
    """把 mock 值转成 ContentBlock 列表（第一周统一 TextContent）。"""
...
def _error_content(msg: str) -> list[types.ContentBlock]:
    """构造错误文本（第一周骨架；后续用 CallToolResult(isError=True) 表达）。"""
