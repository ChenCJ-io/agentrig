...
const runId = "run_reference_regression_20260811";
const cellId = "cell_reference_confirmation_boundary";
const attemptId = "cr_reference_confirmation_boundary_1";
...
test("renders the target overview from authoritative runtime and asset APIs", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Reference HTTP\/SSE Agent 评测总览/ })).toBeVisible();
...
test("uses the migrated asset workbench for cases and execution profiles", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Reference 受控执行" })).toBeVisible();
...
test("connects Run, Cell evidence and acceptance report", async ({ page }) => {
  ...
  await page.getByRole("row").filter({ hasText: "run_refe" }).click();
  await expect(page.getByRole("heading", { name: /运行 run_reference/ })).toBeVisible();
...
async function mockApi(route: Route): Promise<void> {
  ...
  if (path === `/api/targets/${targetId}/check`) return json(route, { reachable: true, driver_type: "http_sse", version: "10.0.0", endpoint: "http://127.0.0.1:8091", capabilities: ["streaming", "tools"], message: "Target 连接正常" });
  ...
  if (path === "/api/execution-profiles") return json(route, page([{ id: "profile_reference", name: "Reference 受控执行", description: "", config: { tool_mode: "controlled", provider_chain: [{ name: "fixture" }], primary_evaluator: "rule", concurrency: 1 }, created_at: createdAt, updated_at: createdAt }]));
...
function target() { return { id: targetId, name: "Reference HTTP/SSE Agent", driver_type: "http_sse", endpoint: "http://127.0.0.1:8091", versions: [{ version: "10.0.0" }], created_at: createdAt, updated_at: createdAt }; }
