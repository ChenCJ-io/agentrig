...
export function AssistantPage() {
  ...
  const createSession = useMutation({
    ...
    onSuccess: async (created) => {
      ...
      setNewTitle("");
      setNotice("评测会话已创建，可以直接提问或描述评测任务。");
    ...
    <div className={styles.workspace}>
      <aside className={styles.sessions}>
        ...
        <footer>
          ...
          <span>
            {assistantAvailable ? "评测助手已就绪" : "智能评测助手不可用"}
          </span>
          <small>
            {assistantAvailable
      ...
      <main className={styles.conversation}>
        <header className={styles.conversationHeader}>
          <div>
            <span className="eyebrow">AgentRig · 智能评测工作台</span>
          ...
          <div className={styles.roomState}>
            ...
            <code>{health.data?.provider ?? "—"}</code>
          </div>
        </header>
      ...
      <aside className={styles.context}>
        ...
        <section className={styles.invocations}>
          <header>
            <div>
              <span className="eyebrow">执行证据入口</span>
              <strong>Run / Cell 证据</strong>
            </div>
            <Badge tone={plan.data?.run_id ? "accent" : "neutral"}>
              {plan.data?.run_id ? 1 : 0}
            </Badge>
          </header>
          <p className={styles.empty}>
            计划提交后，Run、Cell、Attempt 与 Timeline 会成为权威执行证据。
          </p>
        </section>
      </aside>
    </div>
