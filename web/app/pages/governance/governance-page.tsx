...
function ProductionWorkspace({ projectId }: { projectId: string }) {
  ...
  const createDraft = useMutation({
    ...
    onSuccess: (value) => {
      ...
      setNotice(
        `已创建只读 lineage ${shortId(value.lineage.id)}、用例草稿 ${value.case_id}` +
          (value.sample_ids.length
            ? `，并从真实工具调用生成 ${value.sample_ids.length} 个 Sample 草稿。`
            : "。"),
      );
    },
  });
          ...
          {trace ? (
            <>
              ...
              <div className={styles.detailScroll}>
                ...
                {trace.input_full_redacted || trace.input_preview_redacted ? (
                  <section className={styles.timelineSection}>
                    <header>
                      <div>
                        <span className="eyebrow">脱敏对话内容</span>
                        <h3>{trace.input_full_redacted ? "完整正文（已脱敏）" : "截断预览（已脱敏）"}</h3>
                      </div>
                    </header>
                    <pre className={styles.contentBlock}>
                      {trace.input_full_redacted ?? trace.input_preview_redacted}
                    </pre>
                    {trace.output_full_redacted || trace.output_preview_redacted ? (
                      <pre className={styles.contentBlock}>
                        {trace.output_full_redacted ?? trace.output_preview_redacted}
                      </pre>
                    ) : null}
