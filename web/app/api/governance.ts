...
export interface ProductionTrace {
  ...
  input_preview_redacted?: string | null;
  output_preview_redacted?: string | null;
  input_full_redacted?: string | null;
  output_full_redacted?: string | null;
...
export interface IngestSourceIssue {
  source: IngestSource;
  token: string;
}

export interface IngestSourceCreate {
  name: string;
  source_type?: "otlp_http" | "openai_gateway";
  allowed_service_names: string[];
  enabled: boolean;
  retention_days: number;
  redaction_policy?: {
    save_input_preview: boolean;
    save_output_preview: boolean;
  };
  gateway?: {
    upstream_base_url: string;
    upstream_secret_ref: string;
...
): Promise<IngestSourceIssue> {
  return postAction<IngestSourceIssue>(
    projectApiPath(projectId, "/production/ingest-sources"),
    value,
  );
}
