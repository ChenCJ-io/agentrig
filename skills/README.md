...
这组 Skill 指导 Codex、Claude Code 等外部编码 Agent 使用 AgentRig MCP。外部编码 Agent 是
执行控制方：负责选案、提交运行、读取原子证据，并可在关闭 Evidence Judge 时回写自己的判定。
...
Skill 的输入输出 Schema、允许工具与内容哈希由 [`contracts.json`](./contracts.json) 固定，
`scripts/validate_skill_contracts.py` 在 CI 与构建时校验。
